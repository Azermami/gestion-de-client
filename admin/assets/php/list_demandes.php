...
$sql = "SELECT * FROM demande where statut ='Nouveau'" ;
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $sql2 = "SELECT * FROM interpret";
        $result2 = mysqli_query($conn, $sql2);

        if (mysqli_num_rows($result2) > 0) {
            echo '<td><select class="form-select form-select-sm" aria-label="Default select example" name="interpert">';

            while ($row2 = mysqli_fetch_assoc($result2)) {
                echo '<option   value="' . $row2["id_interpret"] . '">' . $row2["nom"] . ' ' . $row2["prenom"] . '</option>';
            }
            echo "</select></td>";

        } else {
            echo "<td>Aucun interprète trouvé.</td>";
